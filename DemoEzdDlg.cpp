...
BEGIN_MESSAGE_MAP(CDemoEzdDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_READPORT, OnButtonReadport)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CDemoEzdDlg::OnButtonRunent() 
{
	...
	double X1=10;
	double Y1=0;
	lmc1_MoveEnt(strName,X1,Y1);
	UpdatePrevBmp();
	AfxMessageBox(_T("Move entity finish!"));	
}
...
void CDemoEzdDlg::OnButtonReadport() 
{
	// TODO: Add your control notification handler code here
	if(lmc1_ReadPort!=NULL)
	{
		WORD data=0;
		if(lmc1_ReadPort(data)==LMC1_ERR_SUCCESS)
		{
			CString str;
		...
		{
			AfxMessageBox(_T("lmc1_ReadPort Failed"));
		}
	}
}
